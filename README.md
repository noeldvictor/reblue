...
| DoF/bloom/flare scheduling | Native atlas/composite and 15 optical sprites in one instanced draw into an explicit persistent output; mirrored quarter-texture sampling preserves the authored glows without asset copies; no supported-path tail-effect/state-308 calls | Three normal startup/transition image/preflight refusals; dual-mask mode, native light/visibility producers, per-eye optics and post/UI image adapters |
...
The latest [native heat-shimmer evidence](research/20260905_1644_native-heat-shimmer.md)
...
protected evidence: 3.89 GiB reclaimed before verification, 87.27 MiB net
saved after the new runs. The historical archive still exceeds its budget;
