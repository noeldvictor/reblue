...
| DoF/bloom/flare scheduling | Native atlas/composite and 15 optical sprites in one instanced draw into an explicit persistent output; mirrored quarter-texture sampling preserves the authored glows without asset copies; no supported-path tail-effect/state-308 calls | Three normal startup/transition image/preflight refusals, native light/visibility producers, per-eye optics and post/UI image adapters |
...
The latest [native directional-bloom evidence](research/20260905_1726_native-directional-bloom.md)
records exact binary/settings, both-eye checks and remaining guest boundaries.
Three superseded normal raw sets were removed, keeping reports/images and
protected evidence: 3.89 GiB reclaimed to fund bounded verification; the
worklog records the measured net change. The historical archive still exceeds its budget;
a frozen inventory and bounded exceptions track the remaining cleanup.
