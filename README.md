...
| DoF production | Complete preparation/submission replacements, native parameters and explicit-input atlas; no supported-path DoF level allocations, quads, intermediate target/resolve or PS c27 reads; enabled by default | Three startup/transition fallbacks per normal tested run, authored camera/focus sources, image/getter adapters, bloom producers/register inputs and outer post scheduling |
...
| Desktop verification | All 26 material/texture/state/camera/post CTests and sixteen source guards pass; 6901 matching DoF parameter checks; latest native-DoF flat/VR captures have 0/31 large jumps and correctly crossed first/last VR depth | Disk space limited this checkpoint to 32 frames per mode. Distant VR blur and character-shadow/full-game coverage remain unqualified; the earlier late-scene failure (110/119 large changes) is not superseded |
