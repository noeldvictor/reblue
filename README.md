...
| Scene lifecycle | Host begin/end bodies with explicit persistent colour/depth roles and explicit-source outputs; no engine 16-slot allocation or resolve-source guessing; normal short desktop sequence has no fallback/mismatch or large jumps | Camera/projection/cache execution, state 308, engine descriptors/getters, shared MSAA/scale copies, downstream post/UI tile-chain adapters and broader visual qualification |
