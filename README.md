...
| DoF/bloom/flare scheduling | Native atlas/composite and 15 optical sprites in one instanced draw into an explicit persistent output; mirrored quarter-texture sampling preserves the authored glows without asset copies; no supported-path tail-effect/state-308 calls | Counted packed effects and image/preflight refusals; other filter combinations, dual-mask mode, native light/visibility producers, per-eye optics and post/UI image adapters |
| Optical adjustments | Fisheye and colour inversion fused into one native layered pass; output-aspect-aware curve, explicit attachment records and private input scratch without a seed copy or emulated resolve; visible flat/both-eye previews verified | Authored activation/parameter comparisons and event coverage, packed/intervening filters and VR comfort |
| Scanline filter | Native four-tap layered pass after optical adjustments, with host-frame animation independent of gameplay RNG; removes the native scope's last guest filter wrapper/state setters; existing noise-off default retained | Authored activation/strength comparison and event coverage; combined animated VR preview has 1/31 large changes in visible wave bands, not a normal stability qualification |
...
| Desktop verification | All 26 material/texture/state/camera/post CTests and 26 source guards pass; optical/scanline previews inspected; latest normal flat/VR captures have 0/119 large jumps and correctly crossed first/last VR depth; earlier authored flare/bloom parameter evidence remains documented | Authored adjustment/scanline activation, distant VR blur, per-eye flare optics and character-shadow/full-game coverage remain unqualified; the earlier late-scene failure (110/119 large changes) is not superseded |
