...
| Native sun camera (experimental) | Native orthographic fit, snapshot and sun-volume culling; scoped removal of an obsolete character light-eye cutoff restores Shu's shadow in the normal short flat sequence (0/119 large jumps) | **Disabled by default:** full-frame final-eye capture still has 10/119 large changes during a foreground passage, blur and inconclusive depth. Complete character submission, other shadow modes and broader coverage remain |
...
| Native eye geometry | Shared full runtime extent for scene and final layers; separate authored UI scale; 1440x1584 scene/final-eye pixels verified without letterboxing at XR scale 1.0 | Readable near/far framing and depth, blur, UI/cinema/movie GPU qualification and broader modes; the default XR scale remains 0.65 |
| Desktop verification | All 24 material/texture/state/camera/output CTests and thirteen source guards pass; latest normal flat sequence has 0/119 large jumps; full-size VR scene and final layers have no letterbox | VR still has a foreground passage (10/119 large changes), blur and inconclusive depth. Last late-scene check still fails (110/119 large frame changes); scenery/text and full-game coverage remain unqualified |
