...
| Native sun camera (experimental) | Tested native orthographic fit, snapshot and explicit sun-volume culling; opt-in integration has GPU capture evidence | **Disabled by default:** Shu's cast shadow disappears when enabled. Resolve that regression and qualify normal flat/final-eye sequences before enabling |
...
| Desktop verification | All 23 material/texture/state/camera CTests and twelve source guards pass; prior native shadow lifecycle short flat and 1440x1584 final-eye sequences have 0/119 large jumps | Experimental sun camera is not visually qualified. Last late-scene check still fails (110/119 large frame changes); scenery/text remain unqualified. VR content remains 1440x808 letterboxed/blurred, with inconclusive depth; the default XR scale remains 0.65 |
