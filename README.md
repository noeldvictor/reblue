...
| DoF/bloom scheduling | Native atlas/composite dispatch into an explicit persistent post output, typed bloom weights and no supported-path bloom allocation/blur loops or ms_tex draw trigger; enabled by default | Counted startup packed effects, two image/preflight refusals per normal run, one retained trailing effect and three state-308 calls per tested field frame; other filter combinations, dual-mask mode and post/UI image adapters |
...
| Desktop verification | All 26 material/texture/state/camera/post CTests and nineteen source guards pass; 3642 matching bloom parameter checks; latest native-post flat/VR captures have 0/119 large jumps and correctly crossed first/last VR depth | Distant VR blur and character-shadow/full-game coverage remain unqualified; the earlier late-scene failure (110/119 large changes) is not superseded |
