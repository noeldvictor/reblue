...
| Fixed foveated rendering | fragment density map on the app's own pass measured expensive and ineffective on the 2026-08 frame; `XR_FB_foveation` needs the scene in the XR swapchain. **The size gate was wrong and is fixed (2026-09-04)**: `FoveationWanted` demanded a target 1024 wide, which the multiview scene layer never is (960 on the desktop, 688 on a Quest under `bd_mv_half_width`), so foveation could not fire on the shipping path whatever the device supported. It counts fragments now, with a two-layer target counted twice. **This machine cannot execute it at all** - the desktop GPU reports `fragment density map no`, so the whole path is device-only and unmeasurable here. **Stage 7's first piece shipped 2026-09-04**: `bd_xr_layered_swapchain` makes the runtime's swapchain a two-layer array, one layer an eye, submitted as two projection views by `imageArrayIndex` instead of a side-by-side panel the compositor resamples; the present pass has a `viewMask = 3` twin and mode 3 of the gamma shader, so each output layer reads its own eye's source layer and the flatten is gone. Verified under `tools/xrsim` (with `bd_xr_mirror` off, which is what gates direct present off Android): the presented capture is 960x2160, the two layers differ in 6.5% of pixels with disparity on the near geometry and both are complete eye views. **The present still fits to the game's aspect on this path, and must**: under `bd_mv_half_width` the latched width is halved while `RenderAspect` keeps the full width, so the guest's content is anamorphically squeezed 2:1 and the fit is what undoes it (1.78 into 960x1080 lands on exactly 960x540). Removing the fit filled the eye and stretched every frame 2x vertically; tried and reverted the same day. The letterbox goes when `Output::LatchedFit` takes the runtime's per-eye size, which has to carry the guest's 16:9 2D layout with it. `research/20260904_1130_the-layered-xr-swapchain.md` |
| Occlusion culling | distance cull (`bd_cull_distance`) and, **since 2026-09-03 13:00, the frustum test on the host** (`bd_host_cull`, default on): the guest's own six planes read once per walk, applied to each node's view-space sphere in host floats; `bd_host_cull_diag` runs the guest test beside it and read zero disagreements over 2,897 nodes a frame. The reflection and shadow views (ids 0, 1, 4, 5, 7, 8, 9) still call the guest test (4% of the desktop Draw Thread). **Occlusion culling on by default since 2026-09-04** (`bd_occlusion_cull`, `gpu/occlusion_cull.*`): the walk notes each camera-view node's sphere, the scene pass draws the proxies under occlusion queries at its end, and a node whose proxy drew nothing last frame has its draws dropped by the queue. A village frame notes 84 nodes, reads 33 occluded, skips 30 draws; a per-frame on/off pair differs in 0.30% of pixels against a 1.23% camera-motion floor, and 120 consecutive captures show no popping (0 neighbour jumps over 6%). The proxies are themselves draws with queries, so the desktop - which is not fragment-bound - is not where this is judged |
