...
| Fixed foveated rendering | fragment density map on the app's own pass measured expensive and ineffective; `XR_FB_foveation` needs the scene in the XR swapchain. **Stage 7's first piece shipped 2026-09-04**: `bd_xr_layered_swapchain` makes the runtime's swapchain a two-layer array, one layer an eye, submitted as two projection views by `imageArrayIndex` instead of a side-by-side panel the compositor resamples; the present pass has a `viewMask = 3` twin and mode 3 of the gamma shader, so each output layer reads its own eye's source layer and the flatten is gone. Verified under `tools/xrsim`: the presented capture is 960x2160, the two layers differ in 6.5% of pixels and both are complete eye views. `research/20260904_1130_the-layered-xr-swapchain.md` |
