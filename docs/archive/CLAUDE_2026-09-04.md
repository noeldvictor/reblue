# Archived project notes: former CLAUDE.md

Snapshot preserved on 2026-09-04 when shared instructions moved to
[`AGENTS.md`](../../AGENTS.md). This is historical evidence, **not active agent
instructions**. It intentionally retains superseded plans, contradictory
measurements and original timestamps. Do not treat its "now", "next", device
permissions or defaults as current; do not import this archive into agent startup.
Paths in the original text below are relative to the repository root.

For the current scope and remaining work, read
[`HOST_RENDERER_TRANSITION.md`](../HOST_RENDERER_TRANSITION.md). Read this snapshot
only when investigating a specific historical decision; new findings belong in
`research/`, not in edits to this archive.
...
**Active scope clarified by the owner, 2026-09-04:** all rendering moves to the
host; gameplay remains recompiled. Preserve the art style and readability,
with freedom to change materials, lighting, geometry and assets. Finish the
desktop transition before Quest optimization. The complete requirements and
remaining dependencies are in `docs/HOST_RENDERER_TRANSITION.md`.

**Native mesh assets, 2026-09-04 17:13 (desktop):** `bd_native_meshes` defaults on.
Loaded-model indexed replays and generated LODs import into persistent `.bdmesh`
files and shared host geometry buffers. The flat village run cooked 657 files
(6.8 MB decimal, one 32 MiB GPU arena), with zero import refusals; a later
multiview run loaded all 729 meshes it needed from disk. A flat 120-frame
native/original correctness sequence had no jumps over 6% and no cyan patches.
The guest draw-template producer and packed shader layouts still exist: this is
an asset boundary, not completion of host rendering. The diorama view has a
64-frame lighting flash with native meshes both enabled and disabled; its depth
verdict is inconclusive. `research/20260904_1713_native-mesh-assets-and-capture-ownership.md`.
