// bd_normal_ps_wind (0xBEE9FB4516ADF0EF), as a host shader substituted at link time
...
cbuffer PixelShaderConstants : register(b1, space4)
{
	...
	float4 g_vUV01Scale : packoffset(c53);
};
	...
	out float4 oC0 : SV_Target0)
{
	...
	else
	{
		...
		r10.xy = g_vUV01Scale.yx != c252.xx;
		ps = r10.y + r10.x;
		r4.w = ps;
		p0 = r4.w != 0.0;
		ps = p0 ? 0.0 : 1.0;
		if (p0)
		{
			r1.zw = trunc(r0.xy);
			r1.zw = -r1.wz + r0.yx;
			r1.zw = r1.zw * g_vUV01Scale.yx;
			r0.y = r1.z + g_vUV01Scale.w;
			ps = g_vUV01Scale.z + r1.w;
			r1.z = ps;
		}
		else
		{
		...
		ps = max(r0.y, r0.y);
		r1.w = ps;
		r10.xyzw = tfetch2D(ColorTexture_Texture2DDescriptorIndex, ColorTexture_SamplerDescriptorIndex, r1.zw, float2(0, 0)).xyzw;
	...
	else
	{
		...
		{
			r3.x = c250.x > g_vLightPos2.w;
			r3.yzw = r2.xyz + -g_vLightPos2.xyz;
			r0.yzw = -r2.xyz + g_vLightPos2.xyz;
			r0.x = dot(r0.yzw, r0.yzw);
			r7.x = dot(r3.wyz, r3.wyz);
			...
			if (p0)
			{
				...
				r3.x = dot(-r0.xzy, g_vLightDir2.zxy);
				ps = clamp(rcp(r7.x), FLT_MIN, FLT_MAX);
				r7.x = ps;
				r3.x = saturate(r3.x + -g_vLightParam2.x);
				ps = g_vLightDir2.w * r7.x;
				r7.x = ps;
				r7.x = saturate(r7.x * r3.x);
				r0.w = r7.x * r0.w;
			}
		}
		r3.xyz = r9.xyz + r0.xyz;
		r7.x = dot(r3.xzy, r3.xzy);
		r0.x = dot(r8.xzy, r0.xzy);
		ps = clamp(rsqrt(abs(r7.x)), FLT_MIN, FLT_MAX);
		r7.x = ps;
		r3.xyz = r3.xyz * r7.xxx;
		r0.y = dot(r3.xzy, r8.xzy);
	...
	else
	{
		...
		if (p0)
		{
			r0.yzw = max(-g_vLightDir3.zyx, -g_vLightDir3.zyx);
			ps = max(c252.x, c252.x);
			r0.x = ps;
		}
		else
		{
			...
			if (p0)
			{
				...
				r3.z = dot(-r0.ywz, g_vLightDir3.zxy);
				ps = clamp(rcp(r7.x), FLT_MIN, FLT_MAX);
				r7.x = ps;
		...
		r5.xyz = r9.xyz + r0.yzw;
		r7.x = dot(r5.xzy, r5.xzy);
		r0.y = dot(r8.xzy, r0.ywz);
		ps = clamp(rsqrt(abs(r7.x)), FLT_MIN, FLT_MAX);
		r7.x = ps;
		r5.xyz = r5.xyz * r7.xxx;
		r0.z = dot(r5.xzy, r8.xzy);
		r0.zw = max(r0.yz, c252.yy);
		ps = clamp(log2(abs(r0.w)), FLT_MIN, FLT_MAX);
		r7.x = ps;
		ps = g_vObjectSpecular.w * r7.x;
		r7.x = ps;
		ps = exp2(r7.x);
	...
	if (g_bDiffuse)
	{
		r5.xyz = r7.www * g_vLightDiffuse1.xyz;
		r6.xyz = r5.xyz + g_vLightAmbient.xyz;
		ps = c252.x - r7.y;
		r7.x = ps;
		r5.xyz = r5.yzx * r7.xxx + -r7.xxx;
		r5.xyz = r5.yzx * g_vShadowSubColor.www + r7.xxx;
		r3.yzw = r3.yyy * g_vLightDiffuse2.zxy + r6.zxy;
		r0.yzw = r0.yyy * g_vLightDiffuse3.yzx + r3.wyz;
	}
	if (g_bDiffuse)
	{
		r0.yzw = -r5.yzx * g_vShadowSubColor.xyz + r0.wyz;
		r1.xyz = r1.xyz * r0.yzw;
	}
	if (g_bSpecular)
	{
		r7.w = r7.z * r7.y;
		r7.xyz = r3.xxx * g_vLightDiffuse2.xyz;
